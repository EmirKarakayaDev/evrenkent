...
<h1 class="font-serif text-3xl sm:text-4xl font-semibold text-slate-900">{{ $title }}</h1>

@if ($byline)
    <div class="text-base text-brand-600 font-medium mt-3">{{ $byline }}</div>
@endif
...
@if (count($stats) > 0)
    <div class="flex flex-wrap items-center gap-x-6 gap-y-2 mt-6 text-sm text-slate-500">
