...
| 10 | Anasayfa: tek-tür pil vitrini yerine çok raflı keşif sayfası | ✅ Tamamlandı (2026-08-16) | **Madde 8'i değiştirir/geçersiz kılar:** kullanıcı geri bildirimiyle anasayfa modeli değişti — "Kitaplar seçili, pil tıklayınca değişen tek vitrin" yerine **her biri kendi başlığı + "Tümünü Gör" linkiyle alt alta dizilen raflar** tercih edildi (mockup'a değil, doğrudan kullanıcı tercihine dayanıyor — birden fazla yerleşim alternatifi sunuldu, "Zengin 5 raf" seçildi). Anasayfa artık sırasıyla: **Yeni Çıkanlar, Çok Satanlar, Editörün Seçkisi, Fırsatlar** (her biri kendi `BookShelf` sorgusundan, 6'şar kitap) ve **Dergiler** (yayınlanmış son 6 sayı) rafları — boş olan bir raf **hiç render edilmiyor** (ör. ileride Fırsatlar boşalırsa o bölüm sayfadan kaybolur, boş kutu göstermiyor). Üstteki Kitaplar/Dergiler/Sözlükler kutuları artık "seçili" durumu taşımıyor (`?raf=` ile anasayfanın kendisi artık hiç etkileşmiyor) — sadece bilgi/link kutusu, ikisi (Kitaplar/Dergiler) tıklanabilir gerçek link, Sözlükler hâlâ "Yakında". `HomeController::index()` artık parametre almıyor (`Request` bağımlılığı kaldırıldı), tüm rafları tek seferde hazırlayıp view'a geçiyor. **`/kitaplar` katalog sayfasındaki pil-switcher (madde 8) değişmedi** — orası hâlâ "bir rafı seçip tam listesini gör" mantığıyla çalışıyor, sadece anasayfadan kaldırıldı. Doğrulama: `view:cache`/`npm run build` hatasız, **82/82 test yeşil**, curl ile anasayfada 5 rafın da doğru sırayla ve toplam 22 kartla (6+6+4+4+2) render olduğu teyit edildi. |
